...
try {
  ...
  Invoke-RestMethod -Uri "$BaseUrl/api/auth/login" -Method Post -ContentType 'application/json' -Headers @{ 'X-Forwarded-For' = '10.45.45.46' } -Body $studentLoginBody -WebSession $studentSession | Out-Null
  $studentCookieHeader = ($studentSession.Cookies.GetCookies([uri]$BaseUrl) | ForEach-Object { "$($_.Name)=$($_.Value)" }) -join '; '
  $uploadJson = & curl.exe --silent --show-error --fail-with-body -H "Cookie: $studentCookieHeader" -H 'X-Forwarded-For: 10.45.45.47' -F "assignmentId=$assignmentId" -F 'file=@README.md;type=text/plain' "$BaseUrl/api/student/files"
  ...
  $submission = Invoke-WebRequest -Uri "$BaseUrl/api/student/assignments" -Method Post -ContentType 'application/json' -Headers @{ 'X-Forwarded-For' = '10.45.45.48' } -Body $submissionBody -WebSession $studentSession -UseBasicParsing
  ...
  Invoke-RestMethod -Uri "$BaseUrl/api/auth/login" -Method Post -ContentType 'application/json' -Headers @{ 'X-Forwarded-For' = '10.45.45.49' } -Body $teacherLoginBody -WebSession $teacherSession | Out-Null
  $download = Invoke-WebRequest -Uri "$BaseUrl$($upload.file.url)" -Headers @{ 'X-Forwarded-For' = '10.45.45.50' } -WebSession $teacherSession -UseBasicParsing
  ...
  Invoke-RestMethod -Uri "$BaseUrl/api/teacher/classes?type=class&classId=$classId" -Method Delete -Headers @{ 'X-Forwarded-For' = '10.45.45.51' } -WebSession $teacherSession | Out-Null
