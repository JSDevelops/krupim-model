...
try {
  ...
  $login = Invoke-RestMethod -Uri "$BaseUrl/api/auth/login" -Method Post -ContentType 'application/json' -Headers @{ 'X-Forwarded-For' = '10.46.46.47' } -Body $loginBody
  $authHeaders = @{ Authorization = "Bearer $($login.access_token)"; 'X-Forwarded-For' = '10.46.46.48' }
  $dataUrl = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII='

  $first = Invoke-RestMethod -Uri "$BaseUrl/api/profile/avatar" -Method Post -ContentType 'application/json' -Headers $authHeaders -Body (@{ dataUrl = $dataUrl } | ConvertTo-Json)
  $firstImage = Invoke-WebRequest -Uri "$BaseUrl$($first.avatarUrl)" -Headers $authHeaders -UseBasicParsing
  if ($firstImage.StatusCode -ne 200 -or -not $firstImage.Headers['Content-Type'].StartsWith('image/png')) { throw 'Avatar download failed' }

  $second = Invoke-RestMethod -Uri "$BaseUrl/api/profile/avatar" -Method Post -ContentType 'application/json' -Headers $authHeaders -Body (@{ dataUrl = $dataUrl } | ConvertTo-Json)
  $oldStatus = 0
  try { Invoke-WebRequest -Uri "$BaseUrl$($first.avatarUrl)" -Headers $authHeaders -UseBasicParsing | Out-Null; $oldStatus = 200 } catch { $oldStatus = [int]$_.Exception.Response.StatusCode }
  if ($oldStatus -ne 404) { throw "Replaced avatar returned $oldStatus instead of 404" }

  Invoke-RestMethod -Uri "$BaseUrl/api/profile/avatar" -Method Delete -Headers $authHeaders | Out-Null
  $removedStatus = 0
  try { Invoke-WebRequest -Uri "$BaseUrl$($second.avatarUrl)" -Headers $authHeaders -UseBasicParsing | Out-Null; $removedStatus = 200 } catch { $removedStatus = [int]$_.Exception.Response.StatusCode }
