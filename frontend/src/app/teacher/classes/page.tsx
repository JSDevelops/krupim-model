...
type ClassInvite = { code: string; className: string; maxUses: number; useCount: number; expiresAt: string; url: string }
...
export default function TeacherClassesPage() {
  ...
  async function createInvite() {
    if (!activeId || !activeClass) return
    setBusy('create-invite')
    try {
      const response = await authenticatedFetch('/api/teacher/classes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'create_invite', classId: activeId, maxUses: 50, expiresInDays: 7 }),
      })
      if (!response.ok) throw new Error(await responseError(response))
      const payload = await response.json() as { invite: Omit<ClassInvite, 'url'> }
      const url = `${window.location.origin}/register-student?code=${encodeURIComponent(payload.invite.code)}`
      setInvite({ ...payload.invite, url })
      toast.success('สร้างลิงก์เชิญแล้ว', { description: `ใช้ได้ 7 วัน สูงสุด ${payload.invite.maxUses} คน` })
    } catch (inviteError) {
      toast.error(inviteError instanceof Error ? inviteError.message : 'สร้างลิงก์เชิญไม่สำเร็จ')
    } finally {
      setBusy(null)
    }
  }
  ...
  async function copyInvite() {
    ...
    try {
      ...
      toast.error('ไม่สามารถคัดลอกอัตโนมัติ กรุณาเลือกลิงก์แล้วคัดลอก')
    }
  }
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.classesWorkspace}>
      ...
      <div className={styles.rosterPanel}>{activeClass ? <><header className={styles.rosterHeader}><div><span><AdminIcon name="users" size={18} /></span><div><h2>{activeClass.name}</h2><p>{activeClass.description || `ปีการศึกษา ${activeClass.year} · ภาคเรียนที่ ${activeClass.semester}`}</p></div></div><div><button type="button" className={styles.secondaryButton} onClick={() => openEdit(activeClass)}><AdminIcon name="edit" size={15} />แก้ไขห้อง</button><button type="button" className={styles.secondaryButton} onClick={() => void createInvite()} disabled={busy === 'create-invite'}><AdminIcon name={busy === 'create-invite' ? 'clock' : 'link'} size={15} />ลิงก์เชิญ</button><button type="button" className={styles.primaryButton} onClick={() => { setMemberEmails(''); setMemberEditorOpen(true) }}><AdminIcon name="plus" size={15} />เพิ่มนักเรียน</button><button type="button" className={styles.dangerButton} onClick={() => void deleteClass(activeClass)}><AdminIcon name="trash" size={15} /></button></div></header><div className={styles.rosterTools}><label className={styles.searchBox}><AdminIcon name="search" size={16} /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="ค้นหาชื่อหรืออีเมล" aria-label="ค้นหานักเรียน" /></label><span>{visibleStudents.length} คน</span></div><div className={styles.studentList}>{rosterLoading ? Array.from({ length: 4 }).map((_, index) => <div className={styles.rowSkeleton} key={index} />) : visibleStudents.length ? visibleStudents.map(student => <article className={styles.studentRow} key={student.id}><span className={styles.studentAvatar}>{initials(student.name)}</span><div><strong>{student.name}</strong><small>{student.email}</small></div><span>{student.schoolName || 'ไม่ระบุสถานศึกษา'}</span><span className={student.status === 'active' ? styles.readyBadge : styles.draftBadge}>{student.status === 'active' ? 'ใช้งาน' : 'ระงับ'}</span><small>{formatDate(student.enrolledAt)}</small><button type="button" onClick={() => void removeStudent(student)} disabled={Boolean(busy)}><AdminIcon name="close" size={15} /></button></article>) : <div className={styles.emptyState}><span><AdminIcon name="student" size={24} /></span><h3>ยังไม่มีนักเรียนในห้องนี้</h3><p>เพิ่มด้วยอีเมลของบัญชีนักเรียนที่เปิดใช้งานแล้ว หรือนำลิงก์เชิญส่งให้นักเรียนสมัครเข้าห้องนี้</p><div><button type="button" onClick={() => void createInvite()}><AdminIcon name="link" size={16} />สร้างลิงก์เชิญ</button><button type="button" onClick={() => setMemberEditorOpen(true)}><AdminIcon name="plus" size={16} />เพิ่มด้วยอีเมล</button></div></div>}</div></> : <div className={styles.emptyState}><span><AdminIcon name="school" size={25} /></span><h3>เริ่มจากสร้างห้องเรียน</h3><p>เมื่อมีห้องเรียนแล้ว คุณจะเพิ่มและจัดการสมาชิกได้ที่นี่</p><button type="button" onClick={openCreate}><AdminIcon name="plus" size={16} />สร้างห้องเรียน</button></div>}</div>
    </section>
    ...
    {invite && <div className={styles.modalOverlay} onMouseDown={event => event.target === event.currentTarget && setInvite(null)}><section className={`${styles.modal} ${styles.compactModal}`} role="dialog" aria-modal="true"><header className={styles.modalHeader}><span><AdminIcon name="link" size={21} /></span><div><h2>ลิงก์เชิญเข้า {invite.className}</h2><p>หมดอายุ {formatDate(invite.expiresAt)} · ใช้ได้สูงสุด {invite.maxUses} คน</p></div><button type="button" onClick={() => setInvite(null)}><AdminIcon name="close" size={18} /></button></header><div className={styles.formGrid}><label className={styles.fullField}><span>รหัสเชิญ</span><input readOnly value={invite.code} /></label><label className={styles.fullField}><span>ลิงก์สำหรับนักเรียน</span><textarea readOnly rows={3} value={invite.url} onFocus={event => event.currentTarget.select()} /></label></div><footer className={styles.modalFooter}><button type="button" onClick={() => setInvite(null)}>ปิด</button><button className={styles.primaryButton} type="button" onClick={() => void copyInvite()}><AdminIcon name="link" size={16} />คัดลอกลิงก์</button></footer></section></div>}
  </main>
}
