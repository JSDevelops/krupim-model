...
export default function ChatPage() {
  ...
  return <><main className={styles.page}><div className={styles.shell}><header className={styles.header}><Link href="/student/explore" className={styles.backButton} aria-label="กลับหน้าสำรวจ"><StudentIcon name="arrowLeft" size={18} /></Link><span className={styles.assistantIcon}><StudentIcon name="sparkles" size={21} /></span><div><h1>ผู้ช่วยการเรียนรู้ AI</h1><p><span />พร้อมช่วยตอบคำถามงานบริการ</p></div></header><section className={styles.chat} aria-label="บทสนทนากับผู้ช่วย AI"><div className={styles.messages} aria-live="polite">{messages.map((message, index) => <article key={`${message.timestamp.getTime()}-${index}`} className={`${styles.messageRow} ${message.role === 'user' ? styles.userRow : styles.aiRow}`}>{message.role === 'model' && <span className={styles.avatar}><StudentIcon name="sparkles" size={16} /></span>}<div className={styles.messageContent}><div className={styles.bubble}>{message.text.split('\n').map((line, lineIndex) => <span key={lineIndex}>{line}{lineIndex < message.text.split('\n').length - 1 && <br />}</span>)}</div><div className={styles.messageMeta}><span>{message.timestamp.toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}</span>{message.role === 'model' && <button type="button" onClick={() => speak(message.text, index)}><StudentIcon name={speakingIndex === index ? 'x' : 'volume'} size={13} />{speakingIndex === index ? 'หยุด' : 'ฟัง'}</button>}</div></div></article>)}{loading && <div className={`${styles.messageRow} ${styles.aiRow}`}><span className={styles.avatar}><StudentIcon name="sparkles" size={16} /></span><div className={styles.typing}><span /><span /><span /></div></div>}<div ref={endRef} /></div>{messages.length < 3 && <div className={styles.suggestions}>{suggestions.map(prompt => <button type="button" key={prompt} onClick={() => void sendMessage(prompt)}>{prompt}</button>)}</div>}<form className={styles.composer} onSubmit={event => { event.preventDefault(); void sendMessage() }}><button type="button" className={`${styles.micButton} ${listening ? styles.listening : ''}`} onClick={toggleMic} aria-label={listening ? 'หยุดรับเสียง' : 'พิมพ์ด้วยเสียง'}><StudentIcon name="mic" size={18} /></button><input value={input} onChange={event => setInput(event.target.value)} placeholder={listening ? 'กำลังรับเสียง...' : 'พิมพ์คำถามหรือประโยคที่ต้องการฝึก...'} maxLength={4000} /><button type="submit" className={styles.sendButton} disabled={!input.trim() || loading} aria-label="ส่งข้อความ"><StudentIcon name="arrowRight" size={18} /></button></form></section></div></main><StudentFINENav /></>
}
